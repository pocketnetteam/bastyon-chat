...
var Process = function (text, chats, parent /* SearchEngine */) {
	...
	var processStopped = function () {
		...
		return (
			!canexecute ||
			stopped ||
			(self.stopcase ? self.stopcase({ timelines, events, results }) : false)
		);
	};

	var stepInChats = function () {
		return f
			.processArray(chats, (e) => {
	...
	var stepInChat = function (chat) {
		...
		return new Promise((resolve, reject) => {
			...
			if (!tl.loadingPromise)
				tl.loadingPromise = tl.line
					.load()
					.then(() => {
	...
	var searchInEvents = function (curevents) {
		return f.clientsearch(text, curevents, (e) => {
			...
			return (
				(e.event.decrypted || e.event.content || {}).body || ""
			).toLowerCase();
		});
	};
