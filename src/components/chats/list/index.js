  ...
  computed: mapState({
    ...
    chats: function (state) {
      ...
      _.each(state.chats, (chat) => {
        ...
        var me = self.core.mtrx.client.credentials.userId

        var users = this.core.mtrx.chatUsersInfo(chat.roomId, 'anotherChatUsers')

        if (users.length === 1 && users[0] && self.core.mtrx.client.isUserIgnored(f.getMatrixIdFull(users[0].id, self.core.domain))) {
