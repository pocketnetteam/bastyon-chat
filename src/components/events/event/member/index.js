...
export default {
	...
	computed: mapState({
		...
		keyword: function () {
			...
			var invitedUserID = f.getmatrixid(
				f.deep(this.event, "event.state_key") || this.event.getSender()
			);
			...
			if (membership === "invite") {
				...
				return (
					this.$i18n.t("caption.hasInvited") +
					`${invitedName}` +
					this.$i18n.t("caption.inTheChat")
				);
			}
			...
			if (membership === "leave") {
				let user =
					this.$store.state.users[f.getmatrixid(this.event.target.userId)];
				if (user) {
					return (
						`${user.name} ` +
						this.$i18n.t("caption.left") +
						this.$i18n.t("caption.theChat")
					);
				}
			}
			...
			if (
				...
				if (!_.isEmpty(pocketUser) && newModer[0]) {
					if (userLevel === 0) {
						return (
							this.$i18n.t("caption.unmarked") +
							` ${pocketUser.name} ` +
							this.$i18n.t("caption.asModerator")
						);
					}
					if (userLevel === 50) {
						return (
							this.$i18n.t("caption.marked") +
							` ${pocketUser.name} ` +
							this.$i18n.t("caption.asModerator")
						);
					}
			...
			if (this.event.event.type === "m.room.name") {
				return (
					this.$i18n.t("caption.editedChatNameTo") +
					`"${this.event.event.content.name}"`
				);
			}
		},
	}),
