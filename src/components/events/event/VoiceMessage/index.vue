...
export default {
  ...
  methods: {
    goTo(e) {
      this.voiceMessage.audio.currentTime =
        (e.offsetX / this.$refs.canvas.width) *
        this.voiceMessage.audio.duration;
      this.voiceMessage.currentTime =
        (e.offsetX / this.$refs.canvas.width) * this.voiceMessage.duration;
      this.draw();
    ...
    setTime() {
      this.voiceMessage.currentTime =
        this.voiceMessage.audio.currentTime * 1000;
    },
    ...
    draw() {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        if (Math.floor(i / 200) === i / 200) {
          // Число 16 для больших аудиофайлов лучше побольше. Нужно подбирать.
