...
class SecretStorage extends _events.EventEmitter {
  ...
  async store(name, secret, keys) {
    ...
    for (const keyId of keys) {
      ...
      if (keyInfo.algorithm === SECRET_STORAGE_ALGORITHM_V1_AES) {
        ...
        _logger.logger.warn(
          "unknown algorithm for secret storage key " +
            keyId +
            ": " +
            keyInfo.algorithm
  ...
  async get(name) {
    ...
    try {
      ...
      if (encInfo.passthrough)
        return (0, olmlib.encodeBase64)(decryption.get_private_key());
  ...
  request(name, devices) {
    ...
    _logger.logger.info(
      `Request secret ${name} from ${devices}, id ${requestId}`
    );
  ...
  async _onRequestReceived(event) {
    ...
    if (content.action === "request_cancellation") {
      if (
        this._incomingRequests[deviceId] &&
        this._incomingRequests[deviceId][content.request_id]
      ) {
        _logger.logger.info(
          "received request cancellation for secret (" +
            sender +
            ", " +
            deviceId +
            ", " +
      ...
      if (secret) {
        ...
        _logger.logger.info(
          `Request denied for ${content.name} secret for ${deviceId}`
        );
      }
    }
  }
  ...
  _onSecretReceived(event) {
    ...
    if (requestControl) {
      ...
      // we requested from
      const deviceInfo =
        this._baseApis._crypto._deviceList.getDeviceByIdentityKey(
      ...
      if (!deviceInfo) {
        _logger.logger.log(
          "secret share from unknown device with key",
          event.getSenderKey()
        );
      ...
      if (!requestControl.devices.includes(deviceInfo.deviceId)) {
        _logger.logger.log(
          "unsolicited secret share from device",
          deviceInfo.deviceId
        );
