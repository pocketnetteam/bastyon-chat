...
OlmEncryption.prototype._ensureSession = function (roomMembers) {
  ...
  const self = this;
  this._prepPromise = self._crypto
...
OlmEncryption.prototype.encryptMessage = async function (
  room,
  eventType,
  content
) {
...
OlmDecryption.prototype.decryptEvent = async function (event) {
  ...
  if (!ciphertext) {
    throw new _base.DecryptionError(
...
OlmDecryption.prototype._reallyDecryptMessage = async function (
  ...
  try {
    ...
    throw new Error(
      "Error decrypting prekey message: " + JSON.stringify(decryptionErrors)
    );
  }
