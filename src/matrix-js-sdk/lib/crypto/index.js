...
function Crypto(
  ...
  this._deviceList = new _DeviceList.DeviceList(
    baseApis,
    cryptoStore,
    this._olmDevice
  ...
  this._outgoingRoomKeyRequestManager =
    new _OutgoingRoomKeyRequestManager.OutgoingRoomKeyRequestManager(
      baseApis,
      this._deviceId,
      this._cryptoStore
  ...
  this._crossSigningInfo = new _CrossSigning.CrossSigningInfo(
    userId,
    cryptoCallbacks,
    cacheCallbacks
  );
  this._secretStorage = new _SecretStorage.SecretStorage(
    baseApis,
    cryptoCallbacks
  );
  ...
  if (
    ...
    cryptoCallbacks.getCrossSigningKey = async (type) => {
      return _CrossSigning.CrossSigningInfo.getFromSecretStorage(
        type,
        this._secretStorage
      );
    };
...
Crypto.prototype.init = async function (opts) {
  const { exportedOlmDevice, pickleKey } = opts || {};
...
Crypto.prototype.bootstrapCrossSigning = async function ({
  ...
  const builder = new _EncryptionSetup.EncryptionSetupBuilder(
    this._baseApis.store.accountData,
    delegateCryptoCallbacks
  );
  const crossSigningInfo = new _CrossSigning.CrossSigningInfo(
    this._userId,
  ...
  const resetCrossSigning = async () => {
    ...
    const deviceSignature = await crossSigningInfo.signDevice(
      this._userId,
      device
    );
  ...
  if (
    ...
    const secretStorage = new _SecretStorage.SecretStorage(
    ...
    if (await secretStorage.hasKey()) {
      _logger.logger.log(
        "Storing new cross-signing private keys in secret storage"
      ); // This is writing to in-memory account data in
      // builder.accountDataClientAdapter so won't fail

      await _CrossSigning.CrossSigningInfo.storeInSecretStorage(
...
Crypto.prototype.bootstrapSecretStorage = async function ({
    ...
    if (
      ...
      try {
        ...
        _logger.logger.error(
          "Signing key backup with cross-signing keys failed",
          e
        );
      }
    } else {
  ...
  const storageExists =
    !setupNewSecretStorage &&
    oldKeyInfo &&
    oldKeyInfo.algorithm === _SecretStorage.SECRET_STORAGE_ALGORITHM_V1_AES; // Log all relevant state for easier parsing of debug logs.
  ...
  if (!storageExists && !keyBackupInfo) {
    ...
    const backupKey =
      ...
      (await getKeyBackupPassphrase()); // create a new SSSS key and use the backup key as the new SSSS key
  ...
  if (
    ...
    _logger.logger.log(
      "Copying cross-signing private keys from cache to secret storage"
    );

    const crossSigningPrivateKeys =
      await this._crossSigningInfo.getCrossSigningKeysFromCache(); // This is writing to in-memory account data in
    // builder.accountDataClientAdapter so won't fail
  ...
  if (setupNewKeyBackup && !keyBackupInfo) {
    ...
    const info = await this._baseApis.prepareKeyBackupVersion(
      null, // don't write to secret storage, as it will write to this._secretStorage.
      /* random key */
    ...
    await secretStorage.store(
      "m.megolm_backup.v1",
      olmlib.encodeBase64(privateKey)
  ...
  if (sessionBackupKey) {
    ...
    if (fixedBackupKey) {
      await secretStorage.store("m.megolm_backup.v1", fixedBackupKey, [
        newKeyId || oldKeyId,
      ]);
    }

    const decodedBackupKey = new Uint8Array(
      olmlib.decodeBase64(fixedBackupKey || sessionBackupKey)
    );
...
Crypto.prototype.checkSecretStoragePrivateKey = function (
  privateKey,
  expectedPublicKey
) {
  let decryption = null;
...
Crypto.prototype.getSessionBackupPrivateKey = async function () {
  ...
  if (key && key.ciphertext) {
    ...
    const decrypted = await (0, _aes.decryptAES)(
      key,
      pickleKey,
      "m.megolm_backup.v1"
    );
...
Crypto.prototype.storeSessionBackupPrivateKey = async function (key) {
  if (!(key instanceof Uint8Array)) {
    throw new Error(
      `storeSessionBackupPrivateKey expects Uint8Array, got ${key}`
    );
  }

  const pickleKey = Buffer.from(this._olmDevice._pickleKey);
  key = await (0, _aes.encryptAES)(
    olmlib.encodeBase64(key),
    pickleKey,
    "m.megolm_backup.v1"
  );
  return this._cryptoStore.doTxn(
    ...
    [_indexeddbCryptoStore.IndexedDBCryptoStore.STORE_ACCOUNT],
    (txn) => {
      this._cryptoStore.storeSecretStorePrivateKey(
        txn,
        "m.megolm_backup.v1",
        key
      );
...
Crypto.prototype.checkCrossSigningPrivateKey = function (
  privateKey,
  expectedPublicKey
) {
  let signing = null;
...
Crypto.prototype._afterCrossSigningLocalKeyChange = async function () {
  ...
  _logger.logger.info(
    `Starting background key sig upload for ${this._deviceId}`
  );

  const upload = ({ shouldEmit }) => {
    return this._baseApis
      .uploadKeySignatures({
        [this._userId]: {
      ...
      .then((response) => {
        ...
        _logger.logger.info(
          `Finished background key sig upload for ${this._deviceId}`
        );
      })
      .catch((e) => {
        _logger.logger.error(
  ...
  if (shouldUpgradeCb) {
    ...
    for (const [userId, crossSigningInfo] of Object.entries(
      ...
      const upgradeInfo = await this._checkForDeviceVerificationUpgrade(
        userId,
        _CrossSigning.CrossSigningInfo.fromStorage(crossSigningInfo, userId)
      );
    ...
    if (Object.keys(users).length > 0) {
      _logger.logger.info(
        `Found ${Object.keys(users).length} verif users to upgrade`
      );
    ...
    if (deviceIds.length) {
      return {
        devices: deviceIds.map((deviceId) =>
          _deviceinfo.DeviceInfo.fromStorage(devices[deviceId], deviceId)
        ),
        crossSigningInfo,
      };
    }
...
Crypto.prototype._checkForValidDeviceSignature = async function (
  userId,
  key,
  devices
) {
...
Crypto.prototype._checkDeviceInfoTrust = function (userId, device) {
  ...
  if (device && userCrossSigning) {
    ...
    // signatures
    const trustCrossSig =
      this._trustCrossSignedDevices || userId === this._userId;
    ...
    return new _CrossSigning.DeviceTrustLevel(
      false,
      false,
      trustedLocally,
      false
...
Crypto.prototype._onDeviceListUserCrossSigningUpdated = async function (
  userId
) {
  if (userId === this._userId) {
    // An update to our own cross-signing key.
    // Get the new key first:
    const newCrossSigning =
      this._deviceList.getStoredCrossSigningForUser(userId);
...
Crypto.prototype.checkOwnCrossSigningTrust = async function ({
  ...
  const crossSigningPrivateKeys =
    await this._crossSigningInfo.getCrossSigningKeysFromCache(); // If we see an update to our own master key, check it against the master
  ...
  if (
    ...
    _logger.logger.info(
      "Attempting to retrieve cross-signing master private key"
    );
  ...
  const selfSigningChanged =
    oldSelfSigningId !== newCrossSigning.getId("self_signing");
  const userSigningChanged =
    oldUserSigningId !== newCrossSigning.getId("user_signing");
  ...
  if (
    ...
    _logger.logger.info(
      "Attempting to retrieve cross-signing self-signing private key"
    );
  ...
  if (
    ...
    _logger.logger.info(
      "Attempting to retrieve cross-signing user-signing private key"
    );
  ...
  if (masterChanged) {
    ...
    const deviceSig =
      masterKey.signatures[this._userId]["ed25519:" + this._deviceId]; // Include only the _new_ device signature in the upload.
  ...
  if (keysToUpload.length) {
    const upload = ({ shouldEmit }) => {
      _logger.logger.info(
        `Starting background key sig upload for ${keysToUpload}`
      );
      ...
      return this._baseApis
        ...
        .then((response) => {
          ...
          _logger.logger.info(
            `Finished background key sig upload for ${keysToUpload}`
          );
...
Crypto.prototype.isKeyBackupTrusted = async function (backupInfo) {
    ...
    if (crossSigningId === sigInfo.deviceId) {
      ...
      try {
        ...
        _logger.logger.warning(
          "Bad signature from cross signing key " + crossSigningId,
          e
        );
  ...
  ret.usable = ret.sigs.some((s) => {
    return (
      s.valid && ((s.device && s.deviceTrust.isVerified()) || s.crossSigningId)
    );
  });
...
Crypto.prototype.setDeviceVerification = async function (
  ...
  if (xsk && xsk.getId() === deviceId) {
    ...
    if (
      !this._crossSigningInfo.getId() &&
      userId === this._crossSigningInfo.userId
    ) {
  ...
  if (verified && userId === this._userId) {
    ...
    if (deviceTrust.isCrossSigningVerified()) {
      _logger.logger.log(
        `Own device ${deviceId} already cross-signing verified`
      );
    } else {
      device = await this._crossSigningInfo.signDevice(
        userId,
        _deviceinfo.DeviceInfo.fromStorage(dev, deviceId)
      );
    }
...
Crypto.prototype.requestVerificationDM = function (userId, roomId) {
  const existingRequest =
    this._inRoomVerificationRequests.findRequestInProgress(roomId);

  if (existingRequest) {
    return Promise.resolve(existingRequest);
  }

  const channel = new _InRoomChannel.InRoomChannel(
    this._baseApis,
    roomId,
    userId
  );
...
Crypto.prototype.requestVerification = function (userId, devices) {
  ...
  const existingRequest =
    this._toDeviceVerificationRequests.findRequestInProgress(userId, devices);

  if (existingRequest) {
    return Promise.resolve(existingRequest);
  }

  const channel = new _ToDeviceChannel.ToDeviceChannel(
    this._baseApis,
    userId,
    devices,
    _ToDeviceChannel.ToDeviceChannel.makeTransactionId()
  );
  return this._requestVerificationWithChannel(
    userId,
    channel,
...
Crypto.prototype._requestVerificationWithChannel = async function (
  ...
  let request = new _VerificationRequest.VerificationRequest(
    channel,
...
Crypto.prototype.beginKeyVerification = function (
  ...
  if (transactionId) {
    ...
    request = new _VerificationRequest.VerificationRequest(
      channel,
      this._verificationMethods,
      this._baseApis
    );
...
Crypto.prototype.legacyDeviceVerification = async function (
  ...
  const request = new _VerificationRequest.VerificationRequest(
    channel,
    this._verificationMethods,
    this._baseApis
  );
...
Crypto.prototype.setRoomEncryption = async function (
  roomId,
  config,
  inhibitDeviceQuery
) {
...
Crypto.prototype.exportRoomKeys = async function () {
  ...
  await this._cryptoStore.doTxn(
    "readonly",
    [_indexeddbCryptoStore.IndexedDBCryptoStore.STORE_INBOUND_GROUP_SESSIONS],
    (txn) => {
...
Crypto.prototype._backupPendingKeys = async function (limit) {
  ...
  for (const session of sessions) {
    ...
    const forwardedCount = (sessionData.forwarding_curve25519_key_chain || [])
      .length;
...
Crypto.prototype.flagAllGroupSessionsForBackup = async function () {
  await this._cryptoStore.doTxn(
    ...
    [
      _indexeddbCryptoStore.IndexedDBCryptoStore.STORE_INBOUND_GROUP_SESSIONS,
      _indexeddbCryptoStore.IndexedDBCryptoStore.STORE_BACKUP,
    ],
    (txn) => {
...
Crypto.prototype.decryptEvent = async function (event) {
  if (event.isRedacted()) {
    const redactionEvent = new _event.MatrixEvent(
...
Crypto.prototype.handleDeviceListChanges = async function (
  syncData,
  syncDeviceLists
) {
...
Crypto.prototype.requestRoomKey = function (
  requestBody,
  recipients,
  resend = false
) {
...
Crypto.prototype._onKeyVerificationMessage = function (event) {
  ...
  const createRequest = (event) => {
    if (
      !_ToDeviceChannel.ToDeviceChannel.canCreateRequest(
        _ToDeviceChannel.ToDeviceChannel.getEventType(event)
      )
    ) {
      return;
    }
    ...
    return new _VerificationRequest.VerificationRequest(
      channel,
      this._verificationMethods,
      this._baseApis
    );
...
Crypto.prototype._processReceivedRoomKeyRequest = async function (req) {
  ...
  if (!(await decryptor.hasKeysForKeyRequest(req))) {
    _logger.logger.log(
      `room key request for unknown session ${roomId} / ` + body.session_id
    );
...
Crypto.prototype._signObject = async function (obj) {
  ...
  sigs[this._userId]["ed25519:" + this._deviceId] = await this._olmDevice.sign(
    _anotherJson.default.stringify(obj)
  );
  obj.signatures = sigs;
