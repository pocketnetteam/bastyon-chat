    ...
    >
      <span class="current">
        ...
        <div
          v-if="enabledFlags"
          class="iti-flag"
          :class="(activeCountry.isohack || activeCountry.iso2).toLowerCase()"
        ></div>
        ...
        <span v-if="enabledCountryCode && source == 'country'" class="country-code">
      ...
      <ul v-show="open" ref="list" class="dropdown-list">
        <li
          ...
          @click="choose(pb)"
          :class="getItemClass(index, pb.isohack || pb.iso2)"
          @mousemove="selectedIndex = index"
        >
          <div
            class="iti-flag"
            v-if="enabledFlags"
            :class="(pb.isohack || pb.iso2).toLowerCase()"
          ></div>
          {{ pb.name }}
          <span v-if="dropdownOptions && !dropdownOptions.disabledDialCode && source == 'country'"
            >+{{ pb.dialCode }}</span
          >
        </li>
      </ul>
...
export default {
  ...
  computed: {
    filteredCountries() {
      ...
      if (this.ignoredCountries.length) {
        return dict[this.source].filter(
          ({ iso2 }) =>
  ...
  methods: {
    ...
    findCountry(iso = "") {
      ...
      return dict[this.source].find(country => country.iso2 === iso);
    },
